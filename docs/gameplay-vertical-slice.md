...
- 首页先流式加载轻量关卡索引并显示真实下载进度；响应总字节可测量时展示百分比与已下载体积，压缩传输、未知总量或完整词库回退时使用不确定进度，不得伪造百分比。下载完成后状态切换为关卡编排，地图可用后立即移除进度条。
...
- 普通关每个记忆串联最多 8 题；离线回退使用当前批次全部可用词，不再收缩为 4 题。
...
- 每个普通关词怪转盘最多展示 8 只词怪；焦点怪最清晰，其他词怪按环形距离逐级缩小、淡化，正后方槽位隐藏，避免八只同权重堆叠。
- AI 阅读链以自然使用 6–8 个目标词为目标；AI 不可用时，离线链直接使用调度出的最多 8 个词，保持一轮挑战的分量一致。
- 每道题是一次攻防结算：
  - 正确：攻击命中；
  - 快速正确：显示暴击；
  - 连续正确：提高连击和表现分；
...
- 所有关卡卡片均可打开词表，锁定状态只限制挑战，不限制查看；普通关显示固定 25 词，Boss 显示其前置普通关组成的综合复盘词池。
- 关卡词表逐词显示义项覆盖率和单词正确率；点击单词后显示全部中文义项，并逐项显示覆盖率、准确率、作答次数和最近练习日期。
- 单词义项覆盖率 = 已有逐义作答记录的义项数 ÷ 该词全部义项数；关卡义项覆盖率按全关所有义项汇总计算，不对单词百分比做简单平均。
- 单词与关卡正确率使用各自全部作答的正确数 ÷ 作答数；逐义准确率只使用该 `senseId` 的作答。未作答时显示“—”，不得伪装为 0% 正确率。
- 单个义项有至少一次逐义作答即视为已覆盖并显示 100%；尚未考查显示 0%。覆盖只代表练过，不代表掌握，也不改变 FSRS 稳定掌握标准。
...
多义词按稳定 `senseId` 进行渐进覆盖，不因词性相同而机械合并：

- 每道选义题最多考查 3 个目标义项，首次按词典顺序从核心义开始；
- 后续复习优先安排从未考过的义项，全部覆盖后优先回补答错率高、最久未考的义项；
- 看义选词与 Boss 反拼必须绑定本轮目标义项，不得永远固定使用第一义；
- 稳定词仍有未覆盖义项时，周期性安排语义题补齐覆盖，但不取消正常的词形提取训练；
- 逐义结果保存在现有单词进度内部；单词仍只拥有一张 FSRS 卡，避免多义词膨胀为数十张独立复习卡；
- 听音选词、听音拼写等纯词形题不计作任何具体义项已掌握。
